...
public class LibroDAOImpl implements LibroDAO {
    ...
    public List<Libro> findAll() {
        return jdbcTemplate.query("select * from biblioteca", new LibroMapper());
    }
    ...
    public Libro findById(Integer idLibro) {
        ...
        args[0] = idLibro;
        return jdbcTemplate.queryForObject("select * from biblioteca where id_libro = ?", args, new LibroMapper());
    }
    ...
    public int update(Libro libro) {
        return jdbcTemplate.update(
                "update biblioteca set anho = ?, titulo = ?,  autor = ?, imprenta = ?, disponible = ? where id_libro = ?",
    ...
    public int delete(Libro libro) {
        return jdbcTemplate.update("delete from biblioteca where id_libro = ?", libro.getIdLibro());
    }
    ...
    public List<Libro> findByTituloOrAutor(String textoBuscado) {
        ...
        args[0] = textoBuscado;
        args[1] = textoBuscado;
        return jdbcTemplate.query("select * from biblioteca where TITULO like ? or AUTOR like ?", args, new LibroMapper());
    }
